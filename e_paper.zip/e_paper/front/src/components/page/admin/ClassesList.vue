  ...
  <div>
    <div class="container">
      <div class="handle-box">
         <el-input v-model="query.name" placeholder="请输入班级名称" class="handle-input mr10"></el-input>
...
export default {
  ...
  methods: {
    ...
    getData() {
        ...
        this.$axios.post('/api/classes/selectPage',param).then(res => {
    ...
    handleAdd() {
        ...
        this.form = {};
        this.$refs['ruleForm'].clearValidate();
        this.dialogName = "班级新增";
    },
    // 保存操作，更新或新增调用
    save(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            this.$axios.post(this.form.id?'/api/classes/edit' : '/api/classes/add', this.form).then(res => {
    ...
    handleDelete(index, row) {
        this.$confirm('确定要删除吗？', '提示', {
           type: 'warning'
             ...
             this.$axios.get('/api/classes/deleteById?id='+row.id).then(res => {
